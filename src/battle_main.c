...
const struct TrainerMoney gTrainerMoneyTable[] =
{
    ...
    {TRAINER_CLASS_AROMA_LADY, 6},
    {TRAINER_CLASS_RUIN_MANIAC, 6},
    {TRAINER_CLASS_INTERVIEWER, 6},
    ...
    {TRAINER_CLASS_BEAUTY, 6},
    {TRAINER_CLASS_RICH_BOY, 10},
    {TRAINER_CLASS_POKEMANIAC, 6},
    {TRAINER_CLASS_SWIMMER_M, 2},
    {TRAINER_CLASS_BLACK_BELT, 6},
    {TRAINER_CLASS_GUITARIST, 6},
    {TRAINER_CLASS_KINDLER, 6},
    {TRAINER_CLASS_CAMPER, 4},
    {TRAINER_CLASS_OLD_COUPLE, 12},
    {TRAINER_CLASS_BUG_MANIAC, 6},
    ...
    {TRAINER_CLASS_HIKER, 6},
    {TRAINER_CLASS_YOUNG_COUPLE, 8},
    {TRAINER_CLASS_WINSTRATE, 6},
...
// Comment out the following line if you have changed .iv to go 0-31, instead of 0-255 as in vanilla.
            ...
            if (FlagGet(FLAG_BRUTAL_MODE_ON) && (gTrainers[trainerNum].trainerClass != TRAINER_CLASS_LEADER) && (gTrainers[trainerNum].trainerClass != TRAINER_CLASS_LELOUCH))
            {
            ...
            if (partyData[i].nature > 0)
                CreateMonWithGenderNatureLetter(&party[i], partyData[i].species, partyData[i].lvl, fixedIV, gender, partyData[i].nature, 0);
            else
            {
...
static void BattleStartClearSetData(void)
{
    ...
    if (!(gBattleTypeFlags & BATTLE_TYPE_RECORDED))
    {
        if (!(gBattleTypeFlags & BATTLE_TYPE_LINK) && gSaveBlock2Ptr->optionsBattleSceneOff == TRUE && FlagGet(FLAG_FORCE_BATTLE_ANIM_ON) == FALSE)
        ...
        case STATE_WAIT_ACTION_CHOSEN: // Try to perform an action.
            if (!(gBattleControllerExecFlags & ((gBitTable[gActiveBattler]) | (0xF << 28) | (gBitTable[gActiveBattler] << 4) | (gBitTable[gActiveBattler] << 8) | (gBitTable[gActiveBattler] << 12))))
            {
                ...
                case B_ACTION_USE_ITEM:
                    if ((FlagGet(FLAG_BRUTAL_MODE_ON) && (gBattleTypeFlags & BATTLE_TYPE_TRAINER)) || (gBattleTypeFlags & (BATTLE_TYPE_LINK
