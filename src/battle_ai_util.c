...
bool32 IsTruantMonVulnerable(u32 battlerAI, u32 opposingBattler)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (gBattleMoves[moves[i]].effect == EFFECT_PROTECT && moves[i] != MOVE_ENDURE)
            return TRUE;
        if (gBattleMoves[moves[i]].effect == EFFECT_SEMI_INVULNERABLE && AI_WhoStrikesFirst(battlerAI, opposingBattler, GetAIChosenMove(battlerAI)) == AI_IS_SLOWER)
            return TRUE;
    }
    return FALSE;
}
...
s32 AI_GetAbility(u32 battlerId)
{
    ...
    if (gSpeciesInfo[gBattleMons[battlerId].species].abilities[0] != ABILITY_NONE)
    {
        ...
        {
            abilityGuess = gSpeciesInfo[gBattleMons[battlerId].species].abilities[Random() % 2];
        }
